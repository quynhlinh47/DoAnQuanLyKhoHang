...
public class ThongKeDAO {
    ...
    public int tinhTongTonKho() {
        ...
        try (Connection conn = MySQLConnection.getConnection()) {
            ...
            ResultSet rs1 = stmt.executeQuery("SELECT SUM(so_luong) FROM nhap_kho");
            if (rs1.next()) tongNhap = rs1.getInt(1);

            ResultSet rs2 = stmt.executeQuery("SELECT SUM(so_luong) FROM xuat_kho");
            if (rs2.next()) tongXuat = rs2.getInt(1);
